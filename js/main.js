...
(function () {
  ...
  const TYPE_FILTERS = [
    { key: "all", label: "全部" },
    { key: "featured", label: "重点案例" },
    { key: "web", label: "网页 Demo" },
    { key: "video", label: "视频演示" },
    { key: "doc", label: "文档演示" },
    { key: "qr", label: "小程序" },
  ];
  const state = { type: "all", cat: "all" };

  function projectType(p) {
    if (p.type === "qr") return "qr";
    if (p.type === "doc") return "doc";
    return p.live === false ? "video" : "web";
  }

  /* ---------- 渲染：分类筛选 ---------- */
  function renderFilters() {
    const typeEl = $("#typeFilters");
    if (typeEl) {
      typeEl.innerHTML = TYPE_FILTERS.map(
        (c, i) => `<button class="chip${i === 0 ? " active" : ""}" type="button" data-type="${esc(c.key)}">${esc(c.label)}</button>`
      ).join("");
    }
    ...
    el.innerHTML = CATEGORIES.map(
      (c, i) => `<option value="${esc(c.key)}"${i === 0 ? " selected" : ""}>${esc(c.label)}</option>`
    ).join("");
  }
  ...
  function renderWebCard(p) {
    ...
    const live = p.type === "doc" ? '<span class="demo-status">文档</span>' : (p.live ? '<span class="demo-status live"><i></i>Live</span>' : '<span class="demo-status">视频</span>');
    const hasUrl = p.url && p.url !== "#";
    const typeLabel = p.type === "doc" ? "文档演示" : (p.live === false ? "视频演示" : "网页 Demo");
    const previewLabel = p.type === "doc" ? "文档演示" : "视频演示";
    const preview = p.live && hasUrl
      ? `<div class="demo-preview"><div class="demo-preview-fallback">${fallbackArt()}</div><iframe class="demo-iframe" data-src="${esc(p.url)}" loading="lazy" scrolling="no" sandbox="allow-scripts allow-same-origin" tabindex="-1" title="${esc(p.title)} preview"></iframe></div>`
      : `<div class="demo-preview demo-preview-static"><div class="demo-preview-fallback">${previewLabel}</div></div>`;
    return `<a class="demo-card reveal" href="${hasUrl ? esc(p.url) : "#"}" data-cat="${esc(p.cat)}" data-type="${projectType(p)}" ${hasUrl ? 'target="_blank" rel="noopener"' : ""}>
      ${preview}
      <div class="demo-card-top">
        <span class="demo-type">${typeLabel}</span>
        ${p.featured ? '<span class="demo-featured">重点</span>' : ""}
      </div>
      ...
      <div class="demo-card-foot">${live}<span class="arrow">${arrow}</span></div>
    </a>`;
  }

  function renderQrCard(p) {
    const tags = (p.tags || []).map((t) => `<span>${esc(t)}</span>`).join("");
    return `<div class="demo-card reveal is-qr" data-cat="${esc(p.cat)}" data-type="qr" data-qr="${esc(p.qr || "")}" data-title="${esc(p.title)}" role="button" tabindex="0" aria-label="${esc(p.title)} 二维码，点击放大">
      <div class="demo-card-top">
        <span class="demo-type">小程序</span>
        ${p.featured ? '<span class="demo-featured">重点</span>' : ""}
      </div>
      <div class="demo-card-body">
        <div>
      ...
      <div class="demo-card-foot"><span class="demo-status wechat"><i></i>扫码体验</span><span class="arrow">${scanIcon}</span></div>
    </div>`;
  }

  function renderDemoCard(p) {
    return p.type === "qr" ? renderQrCard(p) : renderWebCard(p);
  }

  function renderGrid() {
    const el = $("#demoGrid");
    if (!el) return;
    el.innerHTML = PROJECTS.map(renderDemoCard).join("");
    applyDemoFilters();
  }

  function applyDemoFilters() {
    const cards = document.querySelectorAll("#demoGrid .demo-card");
    let count = 0;
    cards.forEach((card) => {
      const typeMatch = state.type === "all" || (state.type === "featured" ? card.querySelector(".demo-featured") : card.dataset.type === state.type);
    ...
    if (countEl) countEl.textContent = count + " 个结果";
  }
  ...
  function bindInteractions() {
    ...
    if (demoRoot) {
      demoRoot.addEventListener("click", (e) => {
        const card = e.target.closest(".demo-card.is-qr");
        if (card) openLB(card.dataset.qr, card.dataset.title);
      });
      demoRoot.addEventListener("keydown", (e) => {
    ...
    document.querySelectorAll("iframe.demo-iframe[data-src]").forEach((f) => {
      f.addEventListener("load", () => { f.classList.add("loaded"); fitPreviews(); });
      ifObs.observe(f);
    });

    const typeFilters = $("#typeFilters");
    if (typeFilters) typeFilters.addEventListener("click", (e) => {
    ...
    if (filters) filters.addEventListener("change", () => {
      state.cat = filters.value || "all";
      applyDemoFilters();
    });
